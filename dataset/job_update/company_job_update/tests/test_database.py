...
from company_job_update.core.frequency_store import FrequencyStore, rebuild_frequency_table
...
def test_review_queue_is_persistent_and_does_not_touch_base_tables(tmp_path) -> None:
    database = tmp_path / "job_update.db"
    store = SQLiteJobUpdateStore(database)
    store.migrate()
    created = store.create_review_item(
        review_id="review_001",
        review_type="job",
        submission_mode="manual",
        job_id="pending_job_001",
        input_payload={"job_title": "AI Engineer"},
        result_payload={"route": {"status": "existing_job"}},
    )

    assert created["status"] == "pending"
    assert created["input"]["job_title"] == "AI Engineer"
    assert len(store.list_review_items()) == 1

    resolved = store.update_review_item(
        "review_001",
        status="merged_existing_job",
        decision_payload={"action": "confirm_existing"},
    )
    assert resolved["decision"]["action"] == "confirm_existing"
