...
class SQLiteJobUpdateStore:
    ...
    def create_review_item(
        ...
        *,
        ...
        """Persist one review task. Review tasks never alter base tables."""
        self.migrate()
        now = _now()
        with self._connect() as conn:
            conn.execute(
                ...
                (review_id, review_type, submission_mode, status, job_id, parent_review_id,
                 input_json, result_json, decision_json, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, '{}', ?, ?)
                """,
                (
                    review_id,
    ...
    def list_review_items(
        ...
        *,
        ...
        with self._connect() as conn:
            rows = conn.execute(
                f"SELECT * FROM review_queue {where} ORDER BY created_at DESC", params
            ).fetchall()
    ...
    def update_review_item(
        ...
        *,
        ...
        if decision_payload is not None:
            ...
            params.append(json.dumps(decision_payload, ensure_ascii=False))
        ...
        with self._connect() as conn:
            cursor = conn.execute(
                f"UPDATE review_queue SET {', '.join(assignments)} WHERE review_id = ?",
                params,
            )
        ...
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS review_queue (
                    ...
                    parent_review_id TEXT NOT NULL DEFAULT '',
...
def _review_row_to_dict(row: sqlite3.Row) -> dict[str, Any]:
    def read_json(value: Any) -> dict[str, Any]:
        try:
    ...
    return {
        ...
        "input": read_json(row["input_json"]),
        "result": read_json(row["result_json"]),
        "decision": read_json(row["decision_json"]),
...
def _create_read_model_indexes(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_review_queue_job_id
            ON review_queue (job_id);
        """
    )
