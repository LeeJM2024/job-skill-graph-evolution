...
class SkillNormalizer:
    def __init__(
        ...
        *,
        ...
        normalized_dictionary: Path | None = None,
        display_dictionary: Path | None = None,
    ) -> None:
    ...
    def _load_normalized_dictionary(self, path: Path | None) -> dict[str, str]:
        if path is not None and path.exists():
            ...
            return result
        return {item["normalized_skill"].casefold(): item["normalized_skill"] for item in self.keyword_map.values()}

    def _load_display_dictionary(self, path: Path | None) -> dict[str, str]:
        if path is not None and path.exists():
            result: dict[str, str] = {}
            with path.open("r", encoding="utf-8-sig", newline="") as handle:
                for row in csv.DictReader(handle):
            ...
            return result
        return {
            item["normalized_skill"].casefold(): item["kg_display_skill"]
            for item in self.keyword_map.values()
